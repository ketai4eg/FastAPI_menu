...
def get_menus_info(db: Session, object_id: int | None = None):
    if object_id is None:
        ...
        menus = db.query(models.Menu).options(
            joinedload(models.Menu.relating_submenu).joinedload(models.SubMenu.relating_dish)).all()
        list_menu = []
        for menu in menus:
            dishes_count = 0
            ...
            menu = schemas.TempMenu(**jsonable_encoder(menu),
        ...
        item = db.query(models.Menu).options(
            joinedload(models.Menu.relating_submenu).joinedload(models.SubMenu.relating_dish)).filter(
            models.Menu.id == object_id).first()
        if item is None:
            raise HTTPException(status_code=404, detail=f'menu not found'.lower())
        dishes_count = 0
        submenu = jsonable_encoder(item)['relating_submenu']
        for dish in submenu:
            dishes_count += len(dish['relating_dish'])
        item = schemas.TempMenu(**jsonable_encoder(item),
...
def get_orm_item(orm_class, object_id, db):
    ...
    if item is None:
        raise HTTPException(status_code=404, detail=f'{orm_class.__name__} not found'.lower())
    return item
...
def get_orm_item_by_name(orm_class, db: Session, title: str):
    ...
    if db_menu:
        raise HTTPException(status_code=400, detail=f'{orm_class.__name__} already existed'.lower())
    return 'ok'
...
def create_submenu(db: Session, menu_id, submenu: schemas.SubMenuCreation):
    new_submenu = models.SubMenu(menu_id=menu_id, title=submenu.title, description=submenu.description)
    db.add(new_submenu)
    db.commit()
...
def get_submenus_list(db: Session, item_id: int):
    submenus = db.query(models.SubMenu).options(joinedload(models.SubMenu.relating_dish)).filter(
        models.SubMenu.menu_id == item_id).all()
    list_submenus = []
    if submenus is None:
        raise HTTPException(status_code=404, detail=f'submenu not found'.lower())
    for submenu in submenus:
        dishes_count = len(jsonable_encoder(submenu)['relating_dish'])
        submenu = schemas.TempSubMenu(**jsonable_encoder(submenu),
                                      dishes_count=dishes_count)
...
def get_submenu_info(db: Session, item_id: int):
    submenu = db.query(models.SubMenu).options(joinedload(models.SubMenu.relating_dish)).filter(
        models.SubMenu.id == item_id).first()
    if submenu is None:
        raise HTTPException(status_code=404, detail=f'submenu not found'.lower())
    dishes_count = len(jsonable_encoder(submenu)['relating_dish'])
    submenu = schemas.TempSubMenu(**jsonable_encoder(submenu),
...
def create_dish(db: Session, submenu_id: int, dish: schemas.DishesBase):
    new_dish = models.Dish(submenu_id=submenu_id, title=dish.title, description=dish.description,
